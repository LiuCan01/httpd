...
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --define "dist .el7.centos.es" --buildsrpm --spec ./SPECS/httpd.spec --sources ./SOURCES/
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --copyout /builddir/build/SRPMS/ ./SRPMS/
/usr/bin/mock -r ${MOCK_CFG} --rootdir ${MOCK_CHROOT} --define "dist .el7.centos.es" --rebuild ./SRPMS/httpd-2.4.6-67.el7.centos.es.1.src.rpm
